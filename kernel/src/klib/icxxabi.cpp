...
extern "C" {
	...
	int __cxa_atexit(void(*f)(void *), void* objptr, void* dso) {
		...
		__atexit_func_count++;
		return 0; /* ABI requires to return 0 on sucess */
	}

//	void __cxa_finalize(void* f) {
//		uarch_t i = __atexit_func_count;
//		if (!f) {
			/** According to the Itanium C++ ABI, if __cxa_finalize called
			 * without a function ptr, then it means that we should destroy everything
			 *
			 * TODO:
...
//				if (__atexit_funcs[i].destructor_func) {
					 ...
					 * will cause undefined behavior. Remember that linear address 0,
